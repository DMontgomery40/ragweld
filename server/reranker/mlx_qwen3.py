...
def resolve_yes_no_token_ids(tokenizer: Any) -> MLXQwen3TokenIds:
    ...
    if len(yes_tokens) != len(suffix_tokens) + 1 or len(no_tokens) != len(suffix_tokens) + 1:
        raise ValueError(
            "Failed to resolve yes/no token ids: tokenization is not single-token beyond suffix. "
            f"len(suffix)={len(suffix_tokens)} len(suffix+yes)={len(yes_tokens)} len(suffix+no)={len(no_tokens)}"
        )
