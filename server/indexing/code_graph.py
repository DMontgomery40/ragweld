...
Cross-file targets (an imported module, a base class or callee defined
elsewhere) are emitted as minimal nodes with the same deterministic id; the
entity upsert is ``MERGE ... SET e += props``, so they unify with the full node
when that file is indexed, in either order. Resolution is deliberately
conservative: a name that cannot be tied to a definition in this file or to an
explicit import that resolves inside the corpus produces no edge and is
...
def module_id(repo_id: str, file_path: str) -> str:
    return f"{repo_id}:{file_path}"


def symbol_id(repo_id: str, file_path: str, qualname: str) -> str:
    return f"{repo_id}:{file_path}::{qualname}"
...
def extract_code_graph(
    *,
    ...
    if lang is None:
        return CodeGraphResult(Neo4jGraph(nodes=[], relationships=[]), lexical_graph_config, [], 0)
    ...
    module_symbol = CodeSymbol("", Path(file_path).name, "module", 1, max(1, source.count("\n") + 1), file_path)
    mod_id = module_id(repo_id, file_path)

    nodes: dict[str, Neo4jNode] = {}
    relationships: list[Neo4jRelationship] = []

    def put_node(node_id: str, label: str, props: dict[str, object]) -> None:
        if node_id not in nodes:
            nodes[node_id] = Neo4jNode(id=node_id, label=label, properties=props)

    def rel(start: str, end: str, rel_type: str, weight: float) -> None:
    ...
    put_node(
        mod_id,
        ENTITY_MODULE,
        {"name": module_symbol.name, "kind": "module", "file_path": file_path, "language": str(language), "start_line": 1, "end_line": module_symbol.end_line, "source": "ast"},
    )
    ...
    for s in facts.symbols:
        sid = symbol_id(repo_id, file_path, s.qualname)
        put_node(
            sid,
            s.label,
            {
        ...
        parent_id = symbol_id(repo_id, file_path, s.parent_qualname) if s.parent_qualname else mod_id
        rel(parent_id, sid, REL_CONTAINS, weights.ast_contains_weight)

    for imported in facts.imported_modules:
        target = module_id(repo_id, imported)
        put_node(target, ENTITY_MODULE, {"name": Path(imported).name, "kind": "module", "file_path": imported, "source": "ast"})
        rel(mod_id, target, REL_IMPORTS, weights.ast_imports_weight)

    def resolve_name(name: str, *, kinds: tuple[str, ...], preferred_label: str) -> str | None:
        if name in local_by_qualname and local_by_qualname[name].kind in kinds:
            return symbol_id(repo_id, file_path, name)
        locals_ = [s for s in local_by_name.get(name, []) if s.kind in kinds]
        if len(locals_) == 1:
            return symbol_id(repo_id, file_path, locals_[0].qualname)
        imported = facts.imported_names.get(name)
        if imported is not None:
            target_path, original = imported
            target = symbol_id(repo_id, target_path, original)
            put_node(target, preferred_label, {"name": original, "kind": preferred_label, "qualname": original, "file_path": target_path, "source": "ast"})
            return target
        return None

    unresolved_bases: list[str] = []
    for class_qualname, base in facts.inherits:
        target = resolve_name(base, kinds=("class",), preferred_label=ENTITY_CLASS)
        if target is None:
            unresolved_bases.append(f"{class_qualname} -> {base}")
            continue
        rel(symbol_id(repo_id, file_path, class_qualname), target, REL_INHERITS, weights.ast_inherits_weight)

    unresolved_calls: list[str] = []
    seen_calls: set[tuple[str, str]] = set()
    for caller, callee in facts.calls:
        target = resolve_name(callee, kinds=("function", "method"), preferred_label=ENTITY_FUNCTION)
        if target is None:
            unresolved_calls.append(f"{caller} -> {callee}")
            continue
        caller_id = symbol_id(repo_id, file_path, caller)
        if (caller_id, target) in seen_calls or caller_id == target:
            continue
        seen_calls.add((caller_id, target))
        rel(caller_id, target, REL_CALLS, weights.ast_calls_weight)
    ...
    for s in facts.symbols:
        ...
        if ch is not None:
            relationships.append(
                Neo4jRelationship(
                    start_node_id=symbol_id(repo_id, file_path, s.qualname),
