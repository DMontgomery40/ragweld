...
- **The trace panel says when a run came from somewhere else.** With no answer in the conversation yet, the Routing Trace panel falls back to the corpus's most recent run — which a Retrieval-tab search, an MCP probe, or another tab can have produced. It now labels that fallback in place ("This is the most recent run on `<corpus>`, not an answer from this conversation") instead of presenting it silently as this conversation's trace.
...
- **A reasoning model shows its thinking.** With **Show model thinking** on in Chat settings (`ui.chat_stream_include_thinking`), a reasoning model's thinking streams in a collapsible panel above the answer — open while it thinks, folded once (and only once) when the answer starts, never persisted with the conversation. Without it (or for models that share nothing), a stage panel shows **Searching…** then **Generating answer…** with rotating tips, and SSE keepalives hold the connection open so no proxy times out a silent model.
- **Follow, jump, resize.** A streaming answer follows the scroll only while you sit at the bottom — scrolling up keeps your place and coming back resumes it; a **Jump to latest** button opens the newest answer at its start when it is off screen; and the chat workbench, like every major panel, resizes from its bottom-right corner (keyed panels remember their height per viewer, main pane and Dock separately).
- **Feedback carries its evidence.** Helpful / Not helpful record the chunks the answer cited alongside the run, and opening a citation quietly logs a click signal on that chunk for the same run — both feed the learning reranker's triplet mining. A failed, stopped or still-streaming answer offers no rating at all: there is no answer to rate.
...
!!! tip "This panel refreshes with the status poll now"
    Native accounting can settle after indexing ends, so the panel refreshes its saved summaries on the same 30-second status poll. These listing reads are pure reads — they never rewrite a run summary as a side effect of displaying it. Each cell's headline reads `<cost> recorded · <state>` (for example `$0.01 recorded · Complete`), with the full breakdown (frozen estimate, native spend, request census, denominators) behind its **Details** disclosure; see [Native run accounting](../operations/native_costs.md).
...
- **The wizard gates semantic indexing on the same schema review as Indexing.** Step 3 reads the selected corpus's scoped graph policy before anything is estimated: with the semantic policy on and no reviewed schema, the start button refuses in place with a **Review graph schema →** deep link into **RAG → Indexing**, and no index request is ever sent. The estimate consent dialog is the same one the Indexing tab uses — a short headline (files, chunks, estimated cost/time, uncertainty) with every number and the saved estimate assumptions behind an **Estimate details** drawer — and the button shows **Preparing…** while the estimator measures.
...
- **Corpus controls read fresh state when they open.** The corpus picker and the Chat source dropdown force a registry refresh every time they open, and a completed index refreshes the registry before any control can open on the stale list — a corpus indexed by a script or in another session no longer sits there reading "Never indexed". A failed refresh is announced in place (`role=alert`) in the source dropdown instead of silently keeping stale rows.
- **"Delete all unindexed corpora" cannot destroy fresh work.** Cleanup re-checks its candidates against a just-fetched registry snapshot (aborting if that fetch fails), and the server independently refuses with a typed `409 corpus_already_indexed` when a corpus finished indexing between the listing and the delete — the raced corpus is skipped and reconciled on the final refresh. See [Corpus vs repo_id](../guides/corpus.md).
- **The mobile shell keeps its panes usable.** Below 900px the top bar wraps to its natural height instead of clipping a long corpus name, the main pane and the settings rail stack with the pane stack — not the page — owning vertical scroll, the RAG subtabs stay on one horizontally scrollable row, and the navigation drawer still opens below the full wrapped header. The Apply conflict footer and a docked document each get their own minimum space, and the sidepanel resize handle stays disabled through the widths where the header can wrap.
