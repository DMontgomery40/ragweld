...
export interface IndexRunConflictDetail {
  ...
  /** What the holding run last reported doing (its most recent log/progress run event), when it has logged one */
...
export interface AgentTrainRun {
  ...
  /** External experiment identifier in the tracking system when available. */
  tracking_experiment_id?: string | null;
...
export interface Corpus {
  ...
  /** True for a corpus the runtime registers and manages itself -- it carries a system_kind marker and indexes through its own path, so it has no operator-run index run. Today that is the chat Recall corpus and the two Codex session corpora. Operator surfaces that list corpora as work the operator is responsible for exclude these. */
  internal?: boolean;
...
export interface IndexEstimate {
  ...
  /** Figures expected to be described (heuristic: PDF pages x 0.4, rounded; omitted entirely when it rounds to zero), when indexing.figures is enabled */
