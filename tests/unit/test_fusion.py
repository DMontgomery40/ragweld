...
@pytest.mark.asyncio
async def test_search_skips_cache_write_for_empty_results_with_retrieval_errors(monkeypatch) -> None:
    import server.retrieval.fusion as fusion_mod
    from server.models.tribrid_config_model import FusionConfig, TriBridConfig
    ...
    class _FakePostgres:
        ...
        async def get_corpus(self, repo_id: str) -> dict[str, object]:
            _ = repo_id
            return {"embedding_dimensions": 0}

        async def vector_search(self, *_args, **_kwargs):
            raise RuntimeError("vector down")

        async def sparse_search_engine(self, *_args, **_kwargs):
            raise RuntimeError("sparse down")

        async def file_path_search(self, *_args, **_kwargs):
            return []
    ...
    monkeypatch.setattr(fusion_mod, "Embedder", _FakeEmbedder, raising=True)
    monkeypatch.setattr(fusion_mod, "load_scoped_config", _fake_load_scoped_config, raising=True)
    monkeypatch.setattr(fusion_mod.SemanticCacheService, "lookup", _fake_lookup, raising=True)
    monkeypatch.setattr(fusion_mod.SemanticCacheService, "write", _fake_write, raising=True)
    ...
    assert out == []
    assert cache_writes == []
    assert fusion.last_debug.get("cache_write") is False
    assert fusion.last_debug.get("cache_write_skipped") == "retrieval_error_empty_results"
    per_corpus_debug = dict(fusion.last_debug.get("fusion_per_corpus") or {})
    corpus_debug = dict(per_corpus_debug.get("test-corpus") or {})
    assert "vector down" in str(corpus_debug.get("fusion_vector_error") or "")
    assert "sparse down" in str(corpus_debug.get("fusion_sparse_error") or "")
