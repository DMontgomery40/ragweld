...
PRODUCTION_DEFAULTS = {
    ("generation", "gen_model"): "openai.gpt-6-sol",
    ("generation", "enrich_model"): "openai.gpt-6-sol",
    ...
    ("evaluation", "ragas_judge_model"): "openai.gpt-6-sol",
    ("evaluation", "promptfoo_grader_model"): "openai.gpt-6-sol",
}
...
def test_proxmox_production_model_defaults_are_current_and_routable() -> None:
    renderer_source = SCRIPT.read_text(encoding="utf-8")
    rollout_source = PROXMOX_ROLLOUT_PLAN.read_text(encoding="utf-8")
    litellm_config = yaml.safe_load(LITELLM_CONFIG.read_text(encoding="utf-8"))
    routable_aliases = {row["model_name"] for row in litellm_config["model_list"]}
    model_default_keys = {
        "gen_model",
        "enrich_model",
        "default_model",
