...
# Operator-supplied secrets: required at start, never generated by bootstrap-secrets.sh.
PROXMOX_OPERATOR_SUPPLIED_SECRET_FILES = ("tribrid_config.json", "alertmanager-discord-webhook")
PROXMOX_BOOTSTRAP_OUTPUT_FILES = tuple(
    path for path in PROXMOX_REQUIRED_SECRET_FILES if path not in PROXMOX_OPERATOR_SUPPLIED_SECRET_FILES
)
...
def _build_secret_root(tmp_path: Path, *, include_tunnel: bool) -> Path:
    ...
    # Placeholder only: the real webhook URL never enters the repo or its tests.
    _write_private_file(secret_root / "alertmanager-discord-webhook", "contract-only-placeholder\n")
...
def test_proxmox_compose_is_pinned_loopback_only_and_secret_file_backed() -> None:
    ...
    # The production merge keeps Laya loopback-only and hard-capped (no swap headroom).
    laya = services["laya"]
    assert laya["labels"]["io.ragweld.managed"] == "true"
    assert laya["ports"] == [
        {"mode": "ingress", "target": 8000, "published": "58180", "protocol": "tcp", "host_ip": "127.0.0.1"}
    ]
    assert int(laya["mem_limit"]) == 4 * 1024**3
    assert int(laya["memswap_limit"]) == int(laya["mem_limit"])
    assert float(laya["cpus"]) == 4
    assert laya["environment"]["LAYA_MODELS"] == "english"
    assert "laya" not in PROXMOX_PRODUCTION_SERVICES
...
def test_proxmox_caddy_logs_every_site_without_headers_or_credential_parameters() -> None:
    """Every site writes a JSON access log, and no Caddy log line carries a secret.

    Authelia reset/verification links carry `?token=`, the Langfuse OIDC callback
    `?code=&state=`; both the access log and the default (error) logger must redact them.
    """

    blocks = _caddy_named_blocks(PROXMOX_CADDYFILE.read_text(encoding="utf-8"))
    for header in (name for name in blocks if name.startswith("http://")):
        assert "import access_log" in blocks[header], header
    for block in (blocks["(access_log)"], blocks[""]):
        assert "output stderr" in block and "wrap json" in block
        assert "request>headers delete" in block
        for key in ("token", "code", "state", "id_token", "access_token", "refresh_token", "client_secret"):
            assert f"replace {key} REDACTED" in block, key
    assert "log default {" in blocks[""]
...
def test_proxmox_lifecycle_start_runtime_runs_exact_compose_allowlist_and_host_flags(
    ...
    # Laya is never in the `--wait` set: it starts after the platform, best-effort, and the
    # launcher never builds it (the image is built at deploy time).
    assert "laya" not in tokens[13:]
    up_lines = [line for line in docker_lines if " up " in line]
    assert len(up_lines) == 2
    assert shlex.split(up_lines[1]) == [
        "docker",
        "compose",
        "--project-name",
        "ragweld",
    ...
    assert lines.index(compose_up) < lines.index(up_lines[1])
    assert lines.index(up_lines[1]) < lines.index("start --no-docker --no-local-model --no-frontend")
...
def test_proxmox_lifecycle_start_runtime_boots_the_platform_when_laya_cannot_start(
    tmp_path: Path,
) -> None:
    # A missing Laya image (or any Laya start failure) must not take the platform down with
    # it: the launcher warns and still starts the host API.
    repo, log_path = _materialize_proxmox_runtime_repo(tmp_path)
...
if [[ " $* " == *" up -d --no-build laya "* ]]; then
  echo "Error response from daemon: No such image: ragweld-laya:0.3.11" >&2
  exit 18
fi
exit 0
""",
    ...
    assert result.returncode == 0, result.stderr
    lines = log_path.read_text(encoding="utf-8").splitlines()
    assert any(line.endswith(" up -d --no-build laya") for line in lines)
    assert "start --no-docker --no-local-model --no-frontend" in lines
    assert "WARNING: laya did not start; the platform starts without it." in result.stderr
    assert "build laya" in result.stderr
...
def _run_bootstrap(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    # Operator-supplied, like the rendered config: a placeholder, never a real webhook URL.
    _write_private_file(secret_root / "alertmanager-discord-webhook", "contract-only-placeholder\n")
...
def _prepare_service_account_lifecycle(
    ...
    # The test stages the operator-supplied files (rendered config, alert webhook) after
    # bootstrap; in production they are owned by the service account.
    for relative_path in PROXMOX_OPERATOR_SUPPLIED_SECRET_FILES:
        os.chown(secret_root / relative_path, uid, gid)
