...
from server.chat import ragweld_mlx
...
@pytest.mark.asyncio
async def test_clear_cache_cancels_pending_idle_unload_tasks() -> None:
    baseline = set(asyncio.all_tasks())

    model = await ragweld_mlx.get_ragweld_chat_model(base_model="base", adapter_dir="adapter")
    async with model._lock:
        model._schedule_idle_unload_locked(unload_after_sec=60, unload_generation=1)

    await asyncio.sleep(0)
    await ragweld_mlx.clear_cache(adapter_dir="adapter")
    await asyncio.sleep(0)
